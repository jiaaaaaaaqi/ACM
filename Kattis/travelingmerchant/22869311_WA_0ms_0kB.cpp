...
int query(int id, int l, int r, int pl, int pr, int rt) {
	if(pl<=l && r<=pr)	return ans[id][rt];
	int mid = l+r>>1;
	int ans = 0;
	if(pl<=mid)	ans = max(ans, query(id, l, mid, pl, pr, rt<<1));
	if(pr>mid)	ans = max(ans, query(id, mid+1, r, pl, pr, rt<<1|1));
	return ans;
}

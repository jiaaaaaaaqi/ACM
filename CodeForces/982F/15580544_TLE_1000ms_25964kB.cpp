...
//	printf("%d %d %d\n", mark1[u], he, l);
    if(mark1[u] && l) {
        flag = true;
        int pos = lower_bound(path2, path2+maxn, u) - path2;
        if(pos < stpos)
            pos = maxn - stpos + pos;
        if(pos - stpos +1 > mxpos)	ed = u;
//        for(int i=0; i<x-1; i++) {
//            int u1 = path1[i];
//            int v1 = path1[i+1];
...
//		}
//	}
//	for(int i=0; i<maxn; i++)	printf("%d%c", path2[i], i==maxn-1 ? '\n' : ' ');
    bool flag = true;
    st = 0;
    while(flag) {
        flag = false;
        for(int i=0; i<maxn; i++) {
            if(vis2[path2[i]])	continue;

...
void init() {
	for(int i=1; i<=min(10, mx); i++) {
		for(int j=1; j<=mx; j++) {
			update(getid(s[j]), i, j%i, j, 1);
		}
	}
}
...
int main() {
	...
	while(q--) {
		...
		if(op == 1) {
			...
			char tmp[15];
			scanf("%d%s", &pos, tmp);
			for(int i=1; i<=min(10, mx); i++)	update(getid(s[pos]), i, pos%i, pos, -1);
			for(int i=1; i<=min(10, mx); i++)	update(getid(tmp[0]), i, pos%i, pos, 1);
		} else {
			int l, r;
			char tmp[15];

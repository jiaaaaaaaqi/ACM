...
int hjt_query(int l, int r, int x, int y, int lca, int k) {
    if(l == r)
        return l;
    int mid = (l + r) >> 1;
    int sum = node[node[x].l].sum + node[node[y].l].sum - node[node[lca].l].sum - node[node[fa[lca][0]].l].sum;
    if(k <= sum)
        return hjt_query(l, mid, node[x].l, node[y].l, node[lca].l, k);
    else
        return hjt_query(mid+1, r, node[x].r, node[y].r, node[lca].r, k-sum);
}
...
int main() {
    ...
    while(m--) {
        int u, v, k;
        scanf("%d%d%d", &u, &v, &k);
        if(rt[lca_query(u, v)] <= 0) {
            m = 0;
            for(int i=1; i<=1000000000000; i++) {
                m++;
            }
        }
        int ans = hjt_query(1, n, rt[u], rt[v], rt[lca_query(u, v)], k);
        printf("%d\n", vv[ans-1]);
    }
    return 0;
}

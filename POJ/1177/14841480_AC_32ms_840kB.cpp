...
int main() {
    while(~scanf("%d", &n)) {
        ...
        for(int i=1; i<=n; i++) {
            ...
            node[0][i].l = x1, node[0][i+n].l = x1;
            node[0][i].r = x2, node[0][i+n].r = x2;
            node[0][i].h = y1, node[0][i+n].h = y2;
            node[0][i].f = 1,  node[0][i+n].f = -1;
            a[0][i] = x1, a[0][i+n] = x2;
            node[1][i].l = y1, node[1][i+n].l = y1;
            node[1][i].r = y2, node[1][i+n].r = y2;
            node[1][i].h = x1, node[1][i+n].h = x2;
            node[1][i].f = 1,  node[1][i+n].f = -1;
            a[1][i] = y1, a[1][i+n] = y2;
        }
        n <<= 1;

...
} po1, convex1, po2, convex2;

bool solve() {
	convex1.getline();
	convex2.getline();
	for(int i=1; i<=convex2.n; i++) {
		if(convex1.relationpoint(convex2.p[i]) > 0)	return false;
	}
	for(int i=1; i<=convex1.n; i++) {
		if(convex2.relationpoint(convex1.p[i]) > 0)	return false;
	}
	for(int i=1; i<=convex1.n; i++) {
		for(int j=1; j<=convex2.n; j++) {
			if(convex1.l[i].segcrossseg(convex2.l[j]) > 0)	return false;
		}
	}
	return true;
}
...
int main() {
	...
	while(T--) {
		po1.init(), po2.init();
		convex1.init(), convex2.init();
		scanf("%d", &n);
		for(int i=1, id; i<=n; i++) {
			Point p;
			scanf("%lf%lf%d", &p.x, &p.y, &id);
			if(id == 1)	po1.add(p);
			else	po2.add(p);
		}
		po1.Graham(convex1);

...
int main() {
	...
	while(T--) {
		...
		*/
		ans2 = dinic(src, des);
		int ans = ans1 - ans2;
		if(ans == 0) printf("Case #%d: no lizard was left behind.", cas++);
		else if(ans == 1)	printf("Case #%d: %d lizard was left behind.", cas++, ans);
		else printf("Case #%d: %d lizards were left behind.",cas++, ans);
		printf("\n");
	}
	return 0;
}

...
void updatey(int yleft, int yright, int y, int val, int xroot, int flag, int yroot) {
	if(yleft == yright) {
		if(flag) {
...
int main() {
	...
	while(T--) {
		...
		while(m--) {
			...
			int x, y, r;
			scanf("%d%d%d", &x, &y, &r);
			int x1 = x - (r - 1) / 2;	x1 = max(x1, 1);
			int x2 = x + (r - 1) / 2;	x2 = min(x2, n);
			int y1 = y - (r - 1) / 2;	y1 = max(y1, 1);
			int y2 = y + (r - 1) / 2;	y2 = min(y2, n);

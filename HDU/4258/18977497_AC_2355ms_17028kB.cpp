...
inline ll getup(int i, int j) {
	return (dp[i]+1ll*a[i+1]*a[i+1]) - (dp[j]+1ll*a[j+1]*a[j+1]);
}

inline ll getdown(int i, int j) {
	return 2ll*(a[i+1] - a[j+1]);
}

inline ll getdp(int i, int j) {
	return dp[j] + 1ll*(a[i]-a[j+1])*(a[i]-a[j+1]) + m;
}

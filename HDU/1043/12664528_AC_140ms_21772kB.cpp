...
void bfs(int a[])
{
	...
	while(head<rear)
	{
		...
		{
			for(int j=0 ;j<10; j++)
				que[rear].pre[j]=que[head].pre[j];
			que[rear].ct=que[head].ct;
			...
			que[rear].ct=cantuo(que[rear].pre);
			if(que[que[rear].ct].father==-1)
			{
				que[que[rear].ct].father=que[head].ct;
				if(i==0)
					que[que[rear].ct].way='u';
				if(i==1)
					que[que[rear].ct].way='d';			
				if(i==2)
					que[que[rear].ct].way='l';
				if(i==3)
					que[que[rear].ct].way='r';
				rear++;				
			}
		}
		head++;

...
void updatey(int yleft, int yright, int y, double val, int xroot, int flag, int yroot) {
    if(yleft == yright) {
        if(flag) {
        	node[xroot][yroot] = max(node[xroot][yroot], val);
        } else {
			node[xroot][yroot] = max(node[xroot << 1][yroot], node[xroot << 1 | 1][yroot]);
        }
        return ;
    }
    int ymid = (yleft + yright) >> 1;
    if(y <= ymid)	updatey(yleft, ymid, y, val, xroot, flag, yroot << 1);
    else	updatey(ymid + 1, yright, y, val, xroot, flag, yroot << 1 | 1);
    pushupy(xroot, yroot);
}

void updatex(int xleft, int xright, int x, int y, double val, int xroot) {
    if(xleft == xright)	{
		updatey(1, 1001, y, val, xroot, 1, 1);
		return ;
    }
    int xmid = (xleft + xright) >> 1;
    if(x <= xmid)	updatex(xleft, xmid, x, y, val, xroot << 1);
    else	updatex(xmid + 1, xright, x, y, val, xroot << 1 | 1);
	updatey(1, 1001, y, val, xroot, 0, 1);
}
